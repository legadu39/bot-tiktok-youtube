...
| `tools/compositor.py` | ✅ complet | `tools/burner.py`, `tools/graphics.py`, MoviePy, FFmpeg | FIX 2026-04-30 DA Premium : `WordClip` — slots `text_cx/text_cy` ajoutés (centroïde bbox, défaut `w//2`/`h//2`). `compose_frame()` animated-path : `alpha_da=min(1,elapsed×10)` (0→1 en 3 frames), `scale_da=0.80+0.20×raw` (80%→~103%→100%), pivot `abs_pivot = target + text_c*` → `x_pos = abs_pivot - round(text_c* × scale)` — centre absolu invariant quelle que soit la scale. |
