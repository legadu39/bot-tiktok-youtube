...
# ✅ PATCH 2 APPLIQUÉ : Stabilisation CDP & Race Conditions
# - Queue de traitement séquentiel (_frame_queue).
# - Nettoyage des sessions orphelines.
# - Gestion robuste des références de frames (WeakSet).

from __future__ import annotations
import asyncio
import traceback
import weakref # <-- Ajout pour gestion mémoire frames
from typing import Callable, Dict, List, Optional, Set, Any, Tuple # ✅ Tuple ajouté ici
...
class CDPMultiTarget:
    """
    Ecoute Network.* via CDP sessions sur la Page et toutes les Frames.
    ...
    def __init__(self, page: Page, poll_interval: float = 0.5) -> None:
        ...
        self._listeners: Dict[str, Set[Callable[[Dict], None]]] = {ev: set() for ev in self.EVENTS} # P1E: Changement List -> Set
        ...
        # ✅ PATCH: Queue de traitement séquentiel pour éviter les Race Conditions
        # Tuple est maintenant correctement importé pour ce typage :
        self._frame_queue: asyncio.Queue[Tuple[str, Frame]] = asyncio.Queue()
        self._processor_task: Optional[asyncio.Task] = None
        
        # ✅ PATCH: Suivi des frames vivantes pour nettoyage automatique
        self._alive_frames = weakref.WeakSet()
    ...
    async def start(self) -> None:
        ...
        self._processor_task = asyncio.create_task(self._process_frame_queue(), name="cdpmt_frame_processor")
        ...
        # On utilise désormais les méthodes liées à l'instance (self)
        self._on_frame_attached_handler = self._on_frame_attached_sync
        self._on_frame_detached_handler = self._on_frame_detached_sync
    ...
    def on(self, event_name: str, callback: Callable[[Dict], None]) -> None:
        if event_name not in self._listeners:
            ...
            return
        # P1E: Utiliser Set.add() pour une gestion robuste des doublons
    ...
    # --- Gestionnaire de queue séquentiel ---

    async def _process_frame_queue(self):
        """Traite les attachements/détachements de frames séquentiellement."""
        while not self._closed:
            try:
                action, frame = await self._frame_queue.get()
                if action == "attach":
                    await self._do_attach_frame(frame)
                elif action == "detach":
                    await self._do_detach_frame(frame)
                self._frame_queue.task_done()
            except asyncio.CancelledError:
                break
            except Exception as e:
                jlog("cdpmt_queue_error", error=str(e), level="ERROR")

    # Handlers synchrones pour Playwright (mettent en queue)
    
    def _on_frame_attached_sync(self, frame: Frame) -> None:
        if not self._closed:
            self._alive_frames.add(frame)
            self._frame_queue.put_nowait(("attach", frame))

    def _on_frame_detached_sync(self, frame: Frame) -> None:
        if not self._closed:
            self._frame_queue.put_nowait(("detach", frame))

    # Logique d'attachement/détachement (exécutée par le processeur)

    async def _do_attach_frame(self, frame: Frame) -> None:
        if self._closed or not frame: return
        # Vérification supplémentaire : la frame est-elle toujours vivante ?
        if frame.is_detached(): 
            jlog("cdpmt_attach_skipped_detached", url=frame.url, level="DEBUG")
            return
        ...
        try:
            # Vérifier si une session existe déjà pour cette frame
            ...
            elif "Target closed" in msg or "Session closed" in msg:
                 jlog("cdp_attach_frame_closed_during_init", frame_id=frame_id, tag=tag, level="WARN")
            else:
                jlog("cdp_attach_frame_error", frame_id=frame_id, tag=tag, url=frame.url, error=msg, error_type=type(e).__name__, level="ERROR")

    async def _do_detach_frame(self, frame: Frame) -> None:
        ...
        for s_id, s_tag in self._session_tags.items():
            if s_tag == tag: # Comparaison par tag (peut être imprécis si noms dupliqués, mais acceptable ici)
        ...
        if session_to_remove_id and session_to_remove_id in self._sessions:
             ...
             try: 
                 ...
                 jlog("cdpmt_detach_session_error", session_id=session_to_remove_id, tag=tag, error=str(detach_err), level="WARN")
        
        # Nettoyage proactif des orphelins
        self._cleanup_orphans()

    def _cleanup_orphans(self):
        """Supprime les sessions dont les frames n'existent plus."""
        # TODO: Implémenter une logique plus fine si nécessaire.
        # Pour l'instant, on compte sur le WeakSet _alive_frames si on veut aller plus loin,
        # mais le gestionnaire 'framedetached' fait le gros du travail.
        pass
    ...
    async def _attach_all_frames(self) -> None:
        ...
        for fr in frames_to_process:
            ...
            # Ajouter à la liste des frames vivantes
            self._alive_frames.add(fr)
            # Traiter via la logique interne (sans passer par la queue pour l'init initiale, ou via la queue ?)
            # Pour l'init, on peut appeler directement _do_attach_frame pour gagner du temps, 
            # mais passer par la queue est plus sûr.
            self._frame_queue.put_nowait(("attach", fr))
            
        jlog("cdpmt_attach_all_frames_queued")
    ...
    async def _prime_session(self, session: CDPSession, *, tag: str, session_id: str) -> None:
        ...
        def _wire(ev: str) -> Callable[[Dict], None]:
            ...
            def _cdp_callback_wrapper(params: Dict) -> None:
                ...
                listeners_for_event = list(self._listeners.get(ev, set())) # Copier la liste du set
