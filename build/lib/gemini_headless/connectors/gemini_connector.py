...
class GeminiConnector:
    ...
    async def ask(self, prompt: str) -> Tuple[str, Dict[str, Any]]:
        ...
        try:
            ...
            # 5. Semantic Timeout Calculation
            # Heuristique : Plus le prompt est complexe/long, plus la réponse sera longue.
            base_timeout = max(
            ...
            complexity_bonus = 0
            # +1s par tranche de 50 chars
            complexity_bonus += (len(prompt) / 50) * 1000 
            
            # +15s si mots clés complexes
            heavy_keywords = ["code", "refactor", "generate", "list", "compare", "essay", "script", "summary"]
            if any(k in prompt.lower() for k in heavy_keywords):
                complexity_bonus += 15000
                
            final_hard_timeout = int(max(base_timeout, base_timeout + complexity_bonus))
            _jlog(self.logger, "semantic_timeout_calculated", base=base_timeout, bonus=int(complexity_bonus), final=final_hard_timeout)
